...
`;
  ...
  try {
    ...
    if (!parsed) {
      console.log("⚠️ Pattern parse failed, using fallback");

      return {
        pattern: "dynamic_programming",
        confidence: 0.5,
        steps: ["Define state", "Define recurrence", "Solve using DP"],
      };
    }
    ...
    return {
      pattern: "dynamic_programming",
      confidence: 0.5,
      steps: ["Define state", "Define recurrence relation", "Use memoization"],
    };
  }
}
