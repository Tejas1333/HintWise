...
const DEBUG = true;
const log = (...args) => DEBUG && console.log("[Attempt]", ...args);
...
export async function analyzeAttempt(input, state) {
  try {
      ...
      messages: [
        {
          ...
          content: JSON.stringify({
            ...
            userCode: input,
            previous_steps: state.algorithm_steps.slice(0, currentIndex + 1),
          }),
        },
      ],
      temperature: 0.3,
    ...
    return {
      mistake_type: evalData.mistake_type || "unknown",
      feedback: evalData.explanation || "Check your logic carefully.",
      hint: evalData.next_hint || "Think about the next step.",
