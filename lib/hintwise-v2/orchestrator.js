...
import { autoEvaluationEngine } from "./pipeline/autoEvaluationEngine";
import { reflectionEngine } from "./pipeline/reflectionEngine";
import { multiPathEngine } from "./pipeline/multiPathEngine";
...
export async function runHintwisePipeline(
  ...
  if (action === "SHOW_SOLUTION") {
    // 1. Generate solution
    let solution = await fullSolutionEngine({
    ...
    // 🔥 2. AUTO EVALUATION
    const evaluation = await autoEvaluationEngine({
      problem: problemQuery,
      solution,
    ...
    if (!evaluation.is_correct) {
      console.log("Fixing solution:", evaluation.issues);
      solution = evaluation.improved_solution;
    }

    // 🔥 reflection + multi-path (keep your existing)
    const reflection = await reflectionEngine({
      problem: problemQuery,
      state,
    });

    const alternatives = await multiPathEngine({
      problem: problemQuery,
      state,
    });
